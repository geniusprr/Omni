...
export class AiStore {
  ...
  setProvider(input: AiProviderConfigInput): AiSnapshot {
    ...
    const apiKey = input.apiKey?.trim() ? this.encrypt(input.apiKey.trim()) : input.clearApiKey ? null : existing?.apiKey || null
    this.db.prepare(`
  ...
  async sendMessage(input: AiSendInput): Promise<AiSendResult> {
    ...
    if (cached?.response && Number(cached.expiresAt) > Date.now()) {
      ...
      answer = await requestProvider(provider, storedProvider?.baseUrl || provider.baseUrl, apiKey, model, history)
    ...
    const assistantMessage: AiMessage = { id: randomUUID(), role: 'assistant', content: answer, createdAt: Date.now(), cached: fromCache }
    ...
    this.db.prepare('UPDATE conversations SET title = ?, provider_id = ?, model = ?, updated_at = ? WHERE id = ?')
      .run(title, provider.id, model, Date.now(), conversation.id)
    this.publish()
  ...
  private listConversations(): AiConversation[] {
    ...
    `).all() as Array<Omit<AiConversation, 'providerId'> & { providerId: AiProviderId; messageCount: number }>
    return rows.map((row) => ({ ...row, createdAt: Number(row.createdAt), updatedAt: Number(row.updatedAt), messageCount: Number(row.messageCount) }))
  }

  private ensureConversation(id: string | null | undefined, providerId: AiProviderId, model: string) {
    if (id) {
      const existing = this.db.prepare('SELECT id, title, provider_id AS providerId, model, created_at AS createdAt, updated_at AS updatedAt FROM conversations WHERE id = ?').get(id) as AiConversation | undefined
...
async function requestProvider(
  ...
  try {
    ...
    if (provider.id === 'openrouter') {
      headers['HTTP-Referer'] = 'https://kapanis.app'
      headers['X-Title'] = 'kapanış.'
    }
